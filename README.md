<div align="center">

<br/>

# Ricky Johnson

**Data Engineer · AI/ML Engineer · Data Scientist**

<br/>

<!-- Animated contribution snake — hero -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Rickyganta/Rickyganta/output/github-contribution-grid-snake-dark.svg"/>
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Rickyganta/Rickyganta/output/github-contribution-grid-snake.svg"/>
  <img src="https://raw.githubusercontent.com/Rickyganta/Rickyganta/output/github-contribution-grid-snake-dark.svg" width="100%" alt="Contribution snake animation"/>
</picture>

<br/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=2800&pause=1800&color=0969DA&center=true&vCenter=true&width=520&height=40&lines=Hi!+How+may+I+help+you%3F;%F0%9F%A4%96+Data+%26+AI+engineer+at+your+service;Pipelines+%C2%B7+Models+%C2%B7+APIs+%C2%B7+Scale" alt="Greeting"/>

<img src="https://readme-typing-svg.demolab.com?font=Inter&weight=400&size=15&duration=3200&pause=1200&color=656D76&center=true&vCenter=true&width=600&height=30&lines=AWS+%C2%B7+Azure+%C2%B7+GCP+%C2%B7+Kafka+%C2%B7+Hadoop+%C2%B7+LangGraph;LLMs+%C2%B7+RAG+%C2%B7+LangChain" alt="Stack"/>

<br/><br/>

<a href="https://rickyganta.github.io"><img src="https://img.shields.io/badge/Portfolio-58A6FF?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio"/></a>
<a href="mailto:rickyganta@gmail.com"><img src="https://img.shields.io/badge/Email-rickyganta@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/></a>
<a href="https://github.com/Rickyganta"><img src="https://img.shields.io/badge/GitHub-@Rickyganta-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/></a>

<img src="https://komarev.com/ghpvc/?username=Rickyganta&label=Profile%20views&color=161b22&style=flat-square" alt="views"/>

</div>

---

<table>
<tr>
<td width="140" align="center" valign="top">
  <img src="https://avatars.githubusercontent.com/u/178003832?v=4" width="120" style="border-radius:50%;" alt="Ricky Johnson"/>
</td>
<td valign="top">

### Hi, I'm **Ricky Johnson** 👋

**Data Engineer · AI/ML Engineer · Data Scientist**

I design and ship end-to-end data systems — from lakehouse pipelines and statistical models to real-time ML APIs in production.

| | |
|:---|:---|
| 🔭 **Now** | DuckDB lakehouses, dbt pipelines, multi-agent AI & real-time ML |
| 🧠 **Stack** | AWS · Azure · GCP · Kafka · Hadoop · LLMs · RAG · LangChain · LangGraph |
| 📫 **Contact** | [rickyganta@gmail.com](mailto:rickyganta@gmail.com) |
| 🌐 **Site** | [rickyganta.github.io](https://rickyganta.github.io) |

</td>
<td width="280" align="right" valign="top">
  <img src="https://user-images.githubusercontent.com/74038190/229223263-cf2e97a0-9ca6-48ca-9ceb-662c967d74e8.gif" width="260" alt="Coding"/>
</td>
</tr>
</table>

---

### What I do

<table>
<tr>
<td width="33%" align="center">

**🗄️ Data Engineering**

Lakehouse pipelines · ETL/ELT  
AWS · Azure · GCP · Kafka · Hadoop

</td>
<td width="33%" align="center">

**🤖 AI / ML Engineering**

LLMs · RAG · LangChain · LangGraph  
Multi-agent systems · FastAPI · Docker

</td>
<td width="33%" align="center">

**📈 Data Science**

Forecasting · SARIMAX · Prophet  
MLflow · walk-forward validation · DuckDB

</td>
</tr>
</table>

---

### Tech stack

<div align="center">

<img src="https://skillicons.dev/icons?i=aws,azure,gcp,python,docker,kafka&perline=6" alt="Cloud row 1"/>
<br/>
<img src="https://skillicons.dev/icons?i=postgres,redis,git,githubactions,linux,bash&perline=6" alt="Cloud row 2"/>
<br/>
<img src="https://skillicons.dev/icons?i=nginx,sqlite,html,css,js,fastapi&perline=6" alt="Cloud row 3"/>

</div>

<br/>

**☁️ Cloud**
<div align="center">
<img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white" alt="AWS"/>
<img src="https://img.shields.io/badge/Azure-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white" alt="Azure"/>
<img src="https://img.shields.io/badge/GCP-4285F4?style=for-the-badge&logo=googlecloud&logoColor=white" alt="GCP"/>
</div>

**🗄️ Big Data & Pipelines**
<div align="center">
<img src="https://img.shields.io/badge/Apache%20Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white" alt="Kafka"/>
<img src="https://img.shields.io/badge/Apache%20Hadoop-66CCFF?style=for-the-badge&logo=apachehadoop&logoColor=black" alt="Hadoop"/>
<img src="https://img.shields.io/badge/DuckDB-FFF000?style=for-the-badge&logo=duckdb&logoColor=black" alt="DuckDB"/>
<img src="https://img.shields.io/badge/dbt-FF694B?style=for-the-badge&logo=dbt&logoColor=white" alt="dbt"/>
<img src="https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white" alt="GHA"/>
</div>

**🤖 AI · LLMs · Agents**
<div align="center">
<img src="https://img.shields.io/badge/LLMs-412991?style=for-the-badge&logo=openai&logoColor=white" alt="LLMs"/>
<img src="https://img.shields.io/badge/RAG-58A6FF?style=for-the-badge" alt="RAG"/>
<img src="https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white" alt="LangChain"/>
<img src="https://img.shields.io/badge/LangGraph-7C3AED?style=for-the-badge&logo=langchain&logoColor=white" alt="LangGraph"/>
<img src="https://img.shields.io/badge/MLflow-0194E2?style=for-the-badge&logo=mlflow&logoColor=white" alt="MLflow"/>
</div>

**⚙️ Engineering**
<div align="center">
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python"/>
<img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white" alt="FastAPI"/>
<img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" alt="Docker"/>
<img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white" alt="PostgreSQL"/>
<img src="https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white" alt="Redis"/>
</div>

---

### Featured projects

<br/>

<table>
<tr>
<td width="50%" valign="top">

#### [`duckdb-equities-pipeline`](https://github.com/Rickyganta/duckdb-equities-pipeline)
`Data Engineering` · Python · DuckDB · dbt · Streamlit

End-to-end modern data stack. Daily market ingest → DuckDB warehouse → dbt star schema → Streamlit dashboard with backtesting & forecasts.

</td>
<td width="50%" valign="top">

#### [`RealTime-ML-API`](https://github.com/Rickyganta/RealTime-ML-API)
`AI/ML Engineering` · FastAPI · Redis · Postgres · Docker

Production recommendation API — **1M+ requests, 0 failures, p99 &lt; 90ms**. Hybrid CF + content ranking, A/B tests, Prometheus & Locust load tests.

</td>
</tr>
<tr>
<td width="50%" valign="top">

#### [`Multi_Agent_Research_Assistant`](https://github.com/Rickyganta/Multi_Agent_Research_Assistant)
`AI/ML Engineering` · LangGraph · Tavily · Streamlit

Multi-agent research orchestrator with parallel researchers, critic loops, citation validation & cited Markdown reports.

</td>
<td width="50%" valign="top">

#### [`retail-demand-forecasting`](https://github.com/Rickyganta/retail-demand-forecasting)
`Data Science` · SARIMAX · Prophet · MLflow · FastAPI

Retail demand forecasting with walk-forward CV, baseline comparisons, sMAPE/WAPE/MASE metrics & experiment tracking.

</td>
</tr>
<tr>
<td width="50%" valign="top">

#### [`Personal_Website`](https://github.com/Rickyganta/Personal_Website)
`Portfolio` · HTML · GitHub Pages

ML/AI engineer portfolio — projects, skills & professional brand.

</td>
<td width="50%" valign="top">

#### [`rickyganta.github.io`](https://rickyganta.github.io)
`Live` · GitHub Pages

Portfolio live at **[rickyganta.github.io](https://rickyganta.github.io)**

</td>
</tr>
</table>

<br/>

<div align="center">

[![duckdb-equities-pipeline](https://github-readme-stats.vercel.app/api/pin/?username=Rickyganta&repo=duckdb-equities-pipeline&theme=github_dark&hide_border=true&border_radius=10)](https://github.com/Rickyganta/duckdb-equities-pipeline)
[![RealTime-ML-API](https://github-readme-stats.vercel.app/api/pin/?username=Rickyganta&repo=RealTime-ML-API&theme=github_dark&hide_border=true&border_radius=10)](https://github.com/Rickyganta/RealTime-ML-API)

[![Multi_Agent_Research_Assistant](https://github-readme-stats.vercel.app/api/pin/?username=Rickyganta&repo=Multi_Agent_Research_Assistant&theme=github_dark&hide_border=true&border_radius=10)](https://github.com/Rickyganta/Multi_Agent_Research_Assistant)
[![retail-demand-forecasting](https://github-readme-stats.vercel.app/api/pin/?username=Rickyganta&repo=retail-demand-forecasting&theme=github_dark&hide_border=true&border_radius=10)](https://github.com/Rickyganta/retail-demand-forecasting)

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=80&section=footer" width="100%"/>

**Ricky Johnson** · Data Engineer · AI/ML Engineer · Data Scientist

<img src="https://readme-typing-svg.demolab.com?font=Inter&size=13&duration=3000&pause=2000&color=8B949E&center=true&vCenter=true&width=400&lines=Thanks+for+visiting+%F0%9F%91%8B;Open+to+collaboration" alt="Footer"/>

</div>
